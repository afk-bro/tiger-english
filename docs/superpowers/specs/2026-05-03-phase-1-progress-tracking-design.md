...
- `test_transaction_rollback_on_event_log_failure` — **uses real DB constraint violation, not mocks.** Setup: seed `user_activity_log` with a row whose `idempotency_key = "${userId}:unit-3:overview:completed"`. Call `complete_lesson_section(user, "unit-3", "overview")`. The projection insert succeeds (new section), the event log insert violates the unique partial index, the transaction rolls back. Assert `lesson_section_progress` has zero rows for that key. Mirror tests for `submit_exercise_attempt` (force a CHECK violation on `type`) and `review_flashcard` (force a FK violation by passing a non-existent `flashcard_id`).
