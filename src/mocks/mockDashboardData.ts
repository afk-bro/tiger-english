// Mock data for the Gain English dashboard
export interface XPData {
  ...
  xpBreakdown: {
    beginner: { xpPerWord: number; wordsCompleted: number; totalXP: number };
    intermediate: { xpPerWord: number; wordsCompleted: number; totalXP: number };
    expert: { xpPerWord: number; wordsCompleted: number; totalXP: number };
  };
}
...
// Helper functions
export const getXPProgressColor = (percentage: number): string => {
  if (percentage < 30) return 'from-accent-400 to-accent-500'; // Orange
  if (percentage < 70) return 'from-primary-400 to-primary-500'; // Blue
