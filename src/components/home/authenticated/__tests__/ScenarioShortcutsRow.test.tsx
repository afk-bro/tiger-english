...
describe("ScenarioShortcutsRow", () => {
  ...
  it("renders nothing when list is empty", () => {
    const { container } = renderInRouter(
      <ScenarioShortcutsRow scenarios={[]} isLoading={false} />,
    );
  ...
  it("renders nothing when list is null (error or initial)", () => {
    const { container } = renderInRouter(
      <ScenarioShortcutsRow scenarios={null} isLoading={false} />,
    );
    expect(container.firstChild).toBeNull();
  });
  ...
  it("renders a skeleton while loading", () => {
    ...
    expect(
      screen.getByTestId("scenario-shortcuts-skeleton"),
    ).toBeInTheDocument();
  });

  it("renders each scenario as a ScenarioCard and a Browse all link", () => {
    renderInRouter(
      <ScenarioShortcutsRow scenarios={scenarios} isLoading={false} />,
    );
    ...
    expect(screen.getByRole("link", { name: /browse_all/ })).toHaveAttribute(
      "href",
      "/ai-tutor",
    );

    fireEvent.click(linkA);
    expect(mockedReport).toHaveBeenCalledWith("home.scenario_shortcut.click", {
      scenario_slug: "a",
    });
  });
});
